<?php
 ...
 trait FindsTeam
 {
     ...
     /**
      * Trova un team "ORFANO" (senza api_id) nel database.
      * Utile per l'adozione automatica durante la sincronizzazione API ufficiale.
      *
      * @param string|null $name Il nome del team da cercare.
      * @return int|null
      */
     public function findOrphanIdByName(?string $name): ?int
     {
         ...
         // T1: Nome esatto
         $team = Team::whereNull('api_id')->whereRaw('LOWER(name) = ?', [$lowerName])->first();
         if ($team) return $team->id;

         // T2: Short name esatto
         $team = Team::whereNull('api_id')->whereRaw('LOWER(short_name) = ?', [$lowerName])->first();
         if ($team) return $team->id;
