<x-filament-panels::page>
    <div class="space-y-3">
        ...
        {{-- ══════════════════════════════════════════════════════════════════ --}}
        @php 
            $th = \App\Helpers\StepHelper::stepTheme($s7_status);
            $s7_status_label = 'VUOTO';
            $s7_badge_color = 'rose';
            if ($s7_status === 'ok') {
                $s7_status_label = 'COMPLETO';
                $s7_badge_color = 'emerald';
            } elseif ($s7_status === 'partial') {
                $s7_status_label = 'PARZIALE';
                $s7_badge_color = 'amber';
            } elseif ($s7_status === 'blocked') {
                $s7_status_label = 'BLOCCATO';
                $s7_badge_color = 'slate';
            }
        @endphp
        <div x-data="{ open: false }" 
             style="width:100%; border-radius:8px; border:1px solid #e5e7eb; overflow:hidden; box-shadow:0 1px 3px rgba(0,0,0,.07); {{ $th['border_style'] }} margin-bottom: 16px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #ffffff;">
            ...
            <div @click="open = !open" 
                ...
                <div style="display:flex; align-items:center; gap:16px;">
                    ...
                    <!-- Status Badge (Stile Pillola) -->
                    <span style="font-size:0.75rem; font-weight:700; color: {{ $s7_badge_color === 'emerald' ? '#047857' : ($s7_badge_color === 'rose' ? '#b91c1c' : ($s7_badge_color === 'slate' ? '#475569' : '#b45309')) }}; background: {{ $s7_badge_color === 'emerald' ? '#ecfdf5' : ($s7_badge_color === 'rose' ? '#fef2f2' : ($s7_badge_color === 'slate' ? '#f1f5f9' : '#fffbeb')) }}; border:1px solid {{ $s7_badge_color === 'emerald' ? '#a7f3d0' : ($s7_badge_color === 'rose' ? '#fecaca' : ($s7_badge_color === 'slate' ? '#cbd5e1' : '#fde68a')) }}; padding:2px 10px; border-radius:12px; text-transform: uppercase;">
                        {{ $s7_status_label }}
                    </span>
                </div>
                <div style="display:flex; align-items:center; gap:12px;">
            ...
            <div x-show="open" 
                ...
                @if($s7_status === 'blocked')
                    <div style="padding: 16px; background-color: #f8fafc; border-radius: 8px; text-align: center; color: #64748b; font-size: 14px;">
                        Devi completare lo step precedente (6. Anagrafica Calciatori) prima di sbloccare questo step.
                    </div>
                @else
                    <!-- Colonne Cards -->
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin-bottom: 16px;">
                        
                        <!-- CARD 1: COPERTURA API -->
                        <div x-tooltip="'Percentuale di calciatori ufficiali (A listone) agganciati ai dati API-Football.'"
                             style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; display: flex; flex-direction: column; justify-content: space-between; min-height: 150px; text-align: left; cursor: help;">
                            <div>
                                <p style="font-size: 10px; font-weight: 700; color: #94a3b8; text-transform: uppercase; margin: 0 0 8px 0; letter-spacing: 0.05em;">COPERTURA ID API</p>
                                <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
                                    <p style="font-size: 24px; font-weight: 900; color: {{ $pct < 90 ? '#ef4444' : '#10b981' }}; margin: 0;">{{ $playerApi }}</p>
                                    <span style="font-size: 12px; font-weight: 700; color: #64748b;">su {{ $playerFanta }} ({{ $pct }}%)</span>
                                </div>
                                <div style="margin-top: 8px; height: 8px; background-color: #e2e8f0; border-radius: 4px; overflow: hidden; border: 1px solid #e2e8f0;">
                                    <div style="height: 100%; background: {{ $pct < 90 ? 'linear-gradient(to right, #ef4444, #b91c1c)' : 'linear-gradient(to right, #10b981, #059669)' }}; width: {{ $pct }}%; border-radius: 4px;"></div>
                                </div>
                            </div>
                            <p style="font-size: 11px; color: #64748b; margin: 12px 0 0 0; line-height: 1.4;">
                                Target minimo: 90%. Senza ID API, il giocatore non accumulerà statistiche sui match reali.
                            </p>
                        </div>

                        <!-- CARD 2: ULTIMA SINCRONIZZAZIONE -->
                        <div x-tooltip="'Data dell\'ultimo scaricamento intensivo delle rose via API.'"
                             style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; display: flex; flex-direction: column; justify-content: space-between; min-height: 150px; text-align: left; cursor: help;">
                            <div>
                                ...
                                <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
                                    <p style="font-size: 20px; font-weight: 900; color: #1e293b; margin: 0;">
                                        {{ $lastSync ? \Carbon\Carbon::parse($lastSync->created_at)->format('d/m/Y H:i') : 'Mai effettuato' }}
                                    </p>
                                </div>
                            </div>
                            <p style="font-size: 11px; color: #64748b; margin: 12px 0 0 0; line-height: 1.4;">
                                L'aggiornamento massivo mappa i giocatori con l'ID fornito dalle API, incrociando i dati anagrafici.
                            </p>
                        </div>
                    ...
                    <!-- Call to Action -->
                    <div style="display: flex; justify-content: flex-end; margin-top: 16px; padding-top: 16px; border-top: 1px solid #f1f5f9;">
                        <a href="{{ route('filament.admin.pages.sync-api-football') }}" 
                           style="display: inline-flex; align-items: center; justify-content: center; padding: 8px 16px; background-color: #3b82f6; color: #ffffff; font-size: 12px; font-weight: 700; border-radius: 6px; text-decoration: none; box-shadow: 0 2px 4px rgba(59, 130, 246, 0.2); transition: background-color 0.2s;"
                           onmouseover="this.style.backgroundColor='#2563eb'"
                           onmouseout="this.style.backgroundColor='#3b82f6'">
                            Lancia Sync API →
